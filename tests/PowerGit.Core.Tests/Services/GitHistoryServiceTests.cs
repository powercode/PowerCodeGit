...
public sealed class GitHistoryServiceTests
{
    ...
    private static string CreateRepositoryWithCommits(int count, out IReadOnlyList<string> shas)
    {
        ...
        for (var index = 0; index < count; index++)
        {
            ...
            var signature = new Signature("PowerGit", "powergit@example.com", DateTimeOffset.UtcNow.AddMinutes(index));
    ...
    private static string CreateRepositoryWithMessages()
    {
        ...
        var signature = new Signature("PowerGit", "powergit@example.com", DateTimeOffset.UtcNow.AddMinutes(-2));
    ...
    {
        var path = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "PowerGitTests", Guid.NewGuid().ToString("N"));
