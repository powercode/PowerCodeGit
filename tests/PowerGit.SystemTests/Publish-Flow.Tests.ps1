...
Describe 'Update-PowerGitManifest' {
    BeforeAll {
        ...
        $SourceManifest = Join-Path -Path $RepoRoot -ChildPath 'src/PowerGit/PowerGit.psd1'
    }

    BeforeEach {
        # Create a temporary module layout: <temp>/module/0.1.0/PowerGit.psd1
        ...
        Copy-Item -Path $SourceManifest -Destination (Join-Path -Path $VersionedDir -ChildPath 'PowerGit.psd1')

        # Create stub files referenced by the manifest so Update-ModuleManifest validation passes
        $null = New-Item -Path (Join-Path -Path $VersionedDir -ChildPath 'PowerGit.dll') -ItemType File -Force
        $null = New-Item -Path (Join-Path -Path $VersionedDir -ChildPath 'PowerGit.Format.ps1xml') -ItemType File -Force
    }
    ...
    It 'Updates ModuleVersion and renames the directory' {
        ...
        $Manifest = Test-ModuleManifest -Path (Join-Path -Path $NewDir -ChildPath 'PowerGit.psd1')
    ...
    It 'Skips rename when directory already matches version' {
        ...
        $Manifest = Test-ModuleManifest -Path (Join-Path -Path $MatchingDir -ChildPath 'PowerGit.psd1')
...
Describe 'End-to-end publish flow' {
    ...
    It 'Parses tag, patches manifest, and produces a valid module for a stable release' {
        ...
        $Manifest = Test-ModuleManifest -Path (Join-Path -Path $FinalDir -ChildPath 'PowerGit.psd1')
